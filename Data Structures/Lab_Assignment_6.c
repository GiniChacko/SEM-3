...
ROLL NO. : 8942
BATCH: B
TITLE:  Write a Program for Dynamic Implementation of Stack
...
void push(StackLL *ptr, int x)//push function to push the elements to the stack
{

    Node *p;
    p=(Node *)malloc(sizeof(Node));//dynamically allocating
    p->info=x;//setting x as the data to be pushed to the stack
    if(ptr->tos == NULL )
    {
...
int pop(StackLL *ptr)//pop function to pop the element from the stack
{
...
int peek(StackLL *ptr)//peek function to display the top most element present in the stack
{
    ...
    if( ptr->tos== NULL)
    {
        ...
        return ptr->tos->info;//return topmost element 
}

int size(StackLL *ptr)//stack function to display the size of the stack
{
    Node *p=ptr->tos;
    int i=0;
    if(p == NULL)
...
void display(StackLL s)//display function to display all the elements present in the stack
{
...
int main()
{
    ...
    do
    {
        ...
        printf("\nEnter your choice : \n1.Insert Data\n 2.Delete Data\n 3.Data at the top of the stack\n 4.Display \n5.Exit\n");
        scanf("%d",&choice);
